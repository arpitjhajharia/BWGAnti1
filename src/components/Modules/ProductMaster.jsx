...
export const ProductMaster = ({ data, actions, setModal, setActiveQuotesView }) => {
    const { products, skus, vendors, quotesReceived, quotesSent } = data;
    const { settings } = data; // Assuming settings are passed in data
                ...
                {filteredProducts.map(p => {
                    ...
                    return (
                        <div key={p.id} className={`bg-white rounded-lg border border-slate-200 transition-all duration-200 ${isExpanded ? 'ring-2 ring-blue-100 shadow-md' : 'hover:shadow-sm'}`}>
                            ...
                            {isExpanded && (<div className="border-t border-slate-100 bg-slate-50/50 p-5 animate-fade-in"><div className="flex justify-between items-center mb-3"><h4 className="text-xs font-bold uppercase text-slate-500 tracking-wider">SKU Configuration & Pricing</h4><Button size="sm" variant="secondary" onClick={() => setModal({ open: true, type: 'sku', data: { productId: p.id, productName: p.name } })}>+ Add Variant</Button></div>{pSkus.length > 0 ? (<div className="bg-white rounded-lg border border-slate-200 overflow-hidden shadow-sm"><table className="w-full text-sm"><thead className="bg-slate-50 text-xs text-slate-500 font-semibold border-b border-slate-100"><tr><th className="px-4 py-3 text-left">Variant</th><th className="px-4 py-3 text-left">Pack</th><th className="px-4 py-3 text-left">Code</th><th className="px-4 py-3 text-left">Latest Cost</th><th className="px-4 py-3 text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-100">{pSkus.map(s => { const latestBuy = quotesReceived.filter(q => q.skuId === s.id).sort((a, b) => b.createdAt - a.createdAt)[0]; return (<tr key={s.id} className="hover:bg-slate-50"><td className="px-4 py-3 font-medium text-slate-700">{s.variant} {s.flavour && <span className="block text-xs text-slate-400 font-normal">{s.flavour}</span>}</td><td className="px-4 py-3 text-slate-600">{s.packSize}{s.unit} ({s.packType})</td><td className="px-4 py-3 font-mono text-xs text-slate-500">{s.name}</td><td className="px-4 py-3">{latestBuy ? (<div><div className="font-medium text-slate-700">{formatMoney(latestBuy.price, latestBuy.currency)}</div><div className="text-[10px] text-slate-400">via {vendors.find(v => v.id === latestBuy.vendorId)?.companyName}</div></div>) : <span className="text-xs text-slate-400 italic">No quotes</span>}</td><td className="px-4 py-3 text-right"><div className="flex justify-end gap-2"><button onClick={() => setModal({ open: true, type: 'sku', data: s, isEdit: true })} className="p-1 hover:bg-blue-50 rounded text-blue-500"><Icons.Edit className="w-3 h-3" /></button><button onClick={() => actions.del('skus', s.id)} className="p-1 hover:bg-red-50 rounded text-red-500"><Icons.X className="w-3 h-3" /></button></div></td></tr>); })}</tbody></table></div>) : (<div className="p-8 text-center border-2 border-dashed border-slate-200 rounded-lg text-slate-400">No SKUs added yet. Click "Add Variant" to configure.</div>)}</div>)}
                        </div>
                    );
                })}
            </div>
