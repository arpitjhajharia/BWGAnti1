        ...
        switch (modal.type) {
            case 'product': return (
                <div className="space-y-4">
                    <h3 className="font-bold text-lg">{modal.isEdit ? 'Edit' : 'New'} Product</h3>
                    <input placeholder="Product Name" className="w-full p-2 border rounded" value={form.name || ''} onChange={e => setForm({ ...form, name: e.target.value })} />
                    ...
                    <input placeholder="Drive Link" className="w-full p-2 border rounded" value={form.driveLink || ''} onChange={e => setForm({ ...form, driveLink: e.target.value })} />
                </div>
            );
            case 'sku': return (
                <div className="space-y-4">
                    <h3 className="font-bold text-lg">{modal.isEdit ? 'Edit' : 'New'} SKU</h3>
                    <div className="grid grid-cols-2 gap-4">
                        <input placeholder="Variant" className="p-2 border rounded" value={form.variant || ''} onChange={e => setForm({ ...form, variant: e.target.value })} />
                    ...
                    <input type="number" placeholder="Std MOQ" className="w-full p-2 border rounded" value={form.standardMoq || ''} onChange={e => setForm({ ...form, standardMoq: e.target.value })} />
                    <div className="bg-slate-100 p-2 text-xs font-mono rounded text-center font-bold text-slate-600">{form.name}</div>
                </div>
            );
            case 'vendor':
            case 'client': return (
            ...
            case 'task': return (
                <div className="space-y-4">
                    <h3 className="font-bold text-lg">{modal.isEdit ? 'Edit' : 'New'} Task</h3>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <select className="p-2 border rounded" value={form.contextType || ''} onChange={e => { setForm({ ...form, contextType: e.target.value, relatedId: null, relatedName: null }); }}>
                    ...
                    {form.contextType === 'Internal' && (
                        <select className="w-full p-2 border rounded" value={form.taskGroup || ''} onChange={e => setForm({ ...form, taskGroup: e.target.value })}>
                    ...
                    {form.contextType === 'Client' && (
                        <>
                            ...
                            <select className="w-full p-2 border rounded bg-purple-50" value={form.secondaryVendorId || ''} onChange={e => setForm({ ...form, secondaryVendorId: e.target.value })}>
                    ...
                    {form.contextType === 'Vendor' && (
                        <>
                            <select className="w-full p-2 border rounded bg-purple-50" value={form.relatedId || ''} onChange={e => { const v = vendors.find(x => x.id === e.target.value); setForm({ ...form, relatedId: v.id, relatedName: v.companyName }); }}>
                                <option>Select Vendor...</option>
                                {vendors.map(v => <option key={v.id} value={v.id}>{v.companyName}</option>)}
                            </select>
                            <select className="w-full p-2 border rounded bg-blue-50" value={form.secondaryClientId || ''} onChange={e => setForm({ ...form, secondaryClientId: e.target.value })}>
            ...
            case 'quoteSent': return (
                <div className="space-y-4">
                    <h3 className="font-bold text-lg">{modal.isEdit ? 'Edit' : 'New'} {modal.type === 'quoteReceived' ? 'Purchase Quote' : 'Sales Quote'}</h3>
                    <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        {modal.type === 'quoteReceived' ? (
                            <select className="p-2 border rounded" value={form.vendorId || ''} onChange={e => setForm({ ...form, vendorId: e.target.value })}>
                                <option>Select Vendor...</option>
                                {vendors.map(v => <option key={v.id} value={v.id}>{v.companyName}</option>)}
                            </select>
                        ) : (
                            <select className="p-2 border rounded" value={form.clientId || ''} onChange={e => setForm({ ...form, clientId: e.target.value })}>
                                <option>Select Client...</option>
                                {clients.map(c => <option key={c.id} value={c.id}>{c.companyName}</option>)}
                            </select>
                        )}
                        <select className="p-2 border rounded" value={form.skuId || ''} onChange={e => setForm({ ...form, skuId: e.target.value })}>
                            <option>Select SKU...</option>
                            {skus.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                        </select>
                    </div>
                    {(modal.type === 'quoteReceived' || modal.type === 'quoteSent') && (
                    ...
                    {modal.type === 'quoteSent' && form.skuId && (
                        <div className="bg-slate-50 p-2 rounded text-xs border border-slate-200">
                            <p className="font-bold mb-2">Base Cost Link (Select one):</p>
                            <div className="max-h-24 overflow-y-auto">
                                {quotesReceived.filter(q => q.skuId === form.skuId).map(q => {
                                    const v = vendors.find(x => x.id === q.vendorId);
                                    return (
                                        <label key={q.id} className="block cursor-pointer hover:bg-slate-100 p-1">
                                            <input type="radio" name="baseCost" checked={form.baseCostId === q.id} onChange={() => setForm({ ...form, baseCostId: q.id, baseCostPrice: q.price })} />
                                            <span className="ml-2">{q.quoteId} ({v?.companyName}) - {formatMoney(q.price, q.currency)} (MOQ: {q.moq})</span>
                                        </label>
                                    )
                                })}
                            </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <input type="number" placeholder="Price" className="p-2 border rounded" value={modal.type === 'quoteReceived' ? form.price : form.sellingPrice} onChange={e => setForm({ ...form, [modal.type === 'quoteReceived' ? 'price' : 'sellingPrice']: e.target.value })} />
                        <select className="p-2 border rounded" value={form.currency || 'INR'} onChange={e => setForm({ ...form, currency: e.target.value })}>
                            <option>INR</option>
                            <option>USD</option>
                        </select>
                    </div>
                    {modal.type === 'quoteSent' && (
                        <select className="w-full p-2 border rounded mt-4" value={form.status || ''} onChange={e => setForm({ ...form, status: e.target.value })}>
                            <option>Draft</option>
                ...
                <div className="space-y-4 max-h-[75vh] overflow-y-auto pr-2">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input className="w-full p-2 border rounded text-sm" value={form.costTerms || ''} onChange={e => setForm({ ...form, costTerms: e.target.value })} />
                        </div>
                        <div>
                            <label className="text-xs font-bold text-slate-500 uppercase">Price Terms</label>
                            <select className="w-full p-2 border rounded text-sm" value={form.priceTerms || ''} onChange={e => setForm({ ...form, priceTerms: e.target.value })}>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        <div>
                            ...
                            <input type="number" className="w-full p-2 border rounded text-sm" value={form.leadTime || ''} onChange={e => setForm({ ...form, leadTime: e.target.value })} />
                        </div>
                        <div>
                            <label className="text-xs font-bold text-slate-500 uppercase">Shelf Life (Mos)</label>
                            <input type="number" className="w-full p-2 border rounded text-sm" value={form.shelfLife || ''} onChange={e => setForm({ ...form, shelfLife: e.target.value })} />
                        </div>
                        <div>
